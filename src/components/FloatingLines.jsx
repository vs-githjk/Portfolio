/* FloatingLines — adapted from React Bits. Wave-line shader backdrop for the
   Builds section. Loaded lazily (dynamic import) so three.js stays out of the
   main bundle. The layer is pointer-events: none, so pointer tracking listens
   on window instead of the canvas. Colors arrive via linesGradient from tokens. */
...
/* HMR + WebGL context = ghost canvases; force a clean reload instead. */
if (import.meta.hot) {
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    /* The layer never receives pointer events, so track on window. */
  ...
  return <div ref={containerRef} className="lines-canvas" aria-hidden="true" />;
}
