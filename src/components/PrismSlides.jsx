...
import './PrismSlides.css'

/* ─────────────────────────────────────────────────────────────────
 * PrismSlides — the four landing-page product-walkthrough demos,
 * extracted verbatim from HowItWorks.jsx. The scroll-pinned phase
 * scaffolding is replaced by a simple timer-based auto-advance that
 * cross-fades between the four phase demos.
 *
 * The phase demos + every helper they use are copied unchanged; each
 * Phase component takes an `active` prop that drives its internal beat
 * timers, so its scripted timeline only runs while it's the visible slide.
 * ───────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────
 * PrismSlides — parent. Two modes:
 *   mode="auto"   (default) — timer auto-advance + crossfade + dots.
 *   mode="scroll"           — a tall scroll-pinned section; the slide card
 *                             stays pinned while phases scrub by on scroll
 *                             (ported from HowItWorks.jsx's HowItWorksPinned).
 * Only the active demo runs its internal beat timeline (via `active`).
 * ───────────────────────────────────────────────────────────────── */

const SLIDE_COMPONENTS = [Phase1Conversational, Phase2Active, Phase3Agentic, Phase4Yours]

export default function PrismSlides({ mode = 'auto' }) {
  return mode === 'scroll' ? <ScrollSlides /> : <AutoSlides />
}

/* ── Auto mode — the original timer-driven crossfade + dots (unchanged) ── */
function AutoSlides() {
  ...
  return (
    <div className="prism-slides">
      <div className="prism-slides-stage">
        {SLIDE_COMPONENTS.map((Phase, i) => (
          <motion.div
            key={i}
            className="prism-slide"
            initial={false}
            animate={{ opacity: activePhase === i ? 1 : 0 }}
            transition={{ duration: 0.6, ease: 'easeInOut' }}
            style={{
              zIndex: activePhase === i ? 2 : 1,
              pointerEvents: activePhase === i ? 'auto' : 'none',
            }}
          >
            <Phase active={activePhase === i} />
          </motion.div>
        ))}
      </div>

      <div className="prism-slides-dots">
        {SLIDE_COMPONENTS.map((_, i) => (
          <button
            key={i}
            type="button"
            className={`prism-slides-dot${i === activePhase ? ' is-active' : ''}`}
...
/* ── Scroll mode — sticky card, scroll-scrubbed phase crossfade ──
 * Ported from HowItWorks.jsx's HowItWorksPinned + phaseRanges + PhaseDemo,
 * adapted to be standalone (progress is computed against WINDOW scroll, not a
 * `.landing-page` container). NB: `position: sticky` on the pin needs a
 * non-transformed ancestor — don't nest this inside a parent with a CSS
 * `transform` (it would break the pin). */
function ScrollSlides() {
  const sectionRef = useRef(null)

  // progress = 0 when the section top hits the viewport top (pin start),
  // 1 when its bottom reaches the viewport bottom (pin end). Computed manually
  // against window scroll so this works standalone (no scroll container).
  ...
  return (
    <section
      ref={sectionRef}
      className="prism-slides-scroll"
      style={{ height: `calc(100vh * ${PHASES + 1})` }}
    >
      <div className="prism-slides-pin">
        <div className="prism-slides">
          <div className="prism-slides-stage">
            {SLIDE_COMPONENTS.map((Phase, i) => (
              <ScrollPhaseLayer key={i} index={i} phaseFloat={phaseFloat}>
                <Phase active={activePhase === i} />
              </ScrollPhaseLayer>
            ))}
          </div>

          <div className="prism-slides-dots" aria-hidden="true">
            {SLIDE_COMPONENTS.map((_, i) => (
              <span
                key={i}
                className={`prism-slides-dot${i === activePhase ? ' is-active' : ''}`}
...
// Opacity ranges for a phase layer (ported from HowItWorks.phaseRanges). First
// phase never fades in (visible from the top); last phase never fades out
// (stays through unpin). Opacity only — no translate (matches the original
// left-demo crossfade behavior).
function phaseRanges(i) {
  if (i === 0) return { input: [0, 1 - TAU, 1], opacity: [1, 1, 0] }
  if (i === PHASES - 1) return { input: [i - TAU, i, PHASES], opacity: [0, 1, 1] }
  return { input: [i - TAU, i, i + 1 - TAU, i + 1], opacity: [0, 1, 1, 0] }
}

// A crossfading slide layer (ported from HowItWorks.PhaseDemo).
function ScrollPhaseLayer({ index, phaseFloat, children }) {
  const r = phaseRanges(index)
  const opacity = useTransform(phaseFloat, r.input, r.opacity)
  return (
    <motion.div className="prism-slide" style={{ opacity }}>
      {children}
    </motion.div>
  )
}
